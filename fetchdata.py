...
# Futbol takımlarının fikstür bilgisi
    ...
    link = os.path.abspath("r.text").replace("\\","/")          
    with open(link, "w") as f:
        x = time.time()
        f.write(f"{x}")
    path = os.path.abspath("register.json").replace("\\","/")          
...
def fetchMatchData(func1, func2):
    path = os.path.abspath("r.text").replace("\\","/") 
...
def Weather():
    ...
    weatherList = []
    path = os.path.abspath("weather.json").replace("\\","/") 
    if os.path.isfile(path):
        with open(path,'r') as f:
            ...
            path = os.path.abspath("r1.text").replace("\\","/")     
            with open(path, 'w') as f:
               x = time.time()
               f.write(f"{x}") 
    else:
